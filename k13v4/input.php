...
elseif ($det=='inputrkegiatan') {
	include "inputrkegiatan.php";
	exit;
}
...
elseif ($det=='inputrptkeg') {
	include "inputrptkegiatan.php";
	exit;
}
...
elseif ($det=='raportkeg') {
	include "inputraportkeg.php";
	exit;
}
elseif ($det=='inputkkm') {
...
if ($det=='kegiatan') {  
	$showLinkOpr=true;
	$nmTabel="tbkegiatan";
	$nmCaptionTabel="Daftar Kegiatan";
	$nmFieldID="id";
	$sField="nourut,kegiatan";
	$sLebar="3,60,60";
	$sFieldCaption="Nomor Urut,Kegiatan";
	$sqlTabel="select * from $nmTabel";
	//$gField[0]="$"."inp=um412_isiCombo5('select * from program_keahlian','kode_programkeahlian','kode','nama','-Pilih-',$"."r['kode_programkeahlian'],'');";	
}else
if ($det=='subkegiatan') {  
	$showLinkOpr=true;
	$nmTabel="tbsubkegiatan";
	$nmCaptionTabel="Daftar Sub Kegiatan";
	$nmFieldID="id";
	$sField="idkegiatan,idgkegiatan,deskripsi";
	$sLebar="20,20,60";
	$sFieldCaption="Kegiatan,Kelompok,Deskripsi";
	$sqlTabel="select s.*,k.kegiatan,g.kelompok from $nmTabel s left join tbkegiatan k on s.idkegiatan=k.id left join tbgkegiatan g on s.idgkegiatan=g.id";
	
	$gField[0]="$"."inp=um412_isiCombo6('select id,kegiatan from tbkegiatan','idkegiatan');";	
	$gField[1]="$"."inp=um412_isiCombo6('select id,kelompok from tbgkegiatan','idgkegiatan');";	
	$gFieldView[0]="$"."inp=-{'kegiatan'}-;";
	$gFieldView[1]="$"."inp=-{'kelompok'}-;";
	
}else
if ($det=='gkegiatan') {  
	$showLinkOpr=true;
	$nmTabel="tbgkegiatan";
	$nmCaptionTabel="Daftar Kelompok Kegiatan";
	$nmFieldID="id";
	$sField="kelompok";
	$sLebar="60";
	$sFieldCaption="Kelompok";
	$sqlTabel="select * from $nmTabel";
	//$gField[0]="$"."inp=um412_isiCombo5('select * from program_keahlian','kode_programkeahlian','kode','nama','-Pilih-',$"."r['kode_programkeahlian'],'');";	
}elseif ($det=='config') {  
	$nmTabel="tbconfig1";
...
//$gField[0]="$"."inp=um412_isiCombo5('select * from program_keahlian','kode_programkeahlian','kode','nama','-Pilih-',$"."r['kode_programkeahlian'],'');";	
	...
	$nfCSV="data_news_".str_replace("-","",$thpl4).".csv";
	$nmTabel="tbnews";
	...
	$nmFieldID="id";
	$addTbBawah="<br><button class='btn btn-danger btn-sm' href=# onclick=\"bukaAjax('content0','input.php?det=log&op=clearlog');\" onclick><i class='icon-trash'></i>&nbsp; Hapus Log</button>";
...
//$gField[3]="$"."inp=um412_isiCombo5('C:berita','kategori');";
	...
	$nfCSV="data_guru_".str_replace("-","",$thpl4).".csv";
	$nmTabel="guru";
	...
	if ($userType=="admin" ) {
		...
		$i++;
		$sField.=",alloweditrkeg";
		$sFieldCaption.=",Akses Raport Kegiatan";
		$gField[$i]="$"."inp=um412_isiCombo5('R:Boleh;1,Tidak Boleh;0','alloweditrkeg');";		
	}
	...
	if ($op=='itb') {
		...
		//$op="showtable";
		$op="itb";
		echo "<br>Setting berhasil diupdate. <a href='#' onclick=\"bukaAjax('content0','$hal&op=itb"."$addParamAdd');return false;\">Klik di sini</a> untuk refresh ";
	exit;
	}
	$addInput0="
	<style>
	...
	$nfCSV="kelas_".(str_replace(" ","",$kelas))."_kodekelas_".$kode_kelas."_".str_replace("-","",$thpl4).".csv";
	$nmTabel="siswa";
...
elseif ($det=='kelas') { 
	...
	$nfCSV="data_kelas_".str_replace("-","",$thpl4).".csv";
	$nmFieldID="kode";
...
elseif ($det=='mata pelajaran') { //guru
	$nfCSV="data_mata_pelajaran_".str_replace("-","",$thpl4).".csv";
...
elseif ($op=='del') {
	...
	echo "Data Berhasil masuk sebanyak $jrSukses record. <a href='#' onclick=\"bukaAjax('content0','$hal&op=showtable"."$addParamAdd');return false;\">Klik di sini</a> untuk refresh ";
	echo "</div>";
	exit;
...
elseif ($op=='itb') {
	...
	if ($showLinkLihat) {
	$tblihat=" 
	<button href=# class='btn btn-warning btn-sm' onclick=\"bukaAjax('content0','$hal".$addParamAdd."');return false;\">
...
$lh=buatlinkPage3("tcari",$sqlall,$nfrep."&noth=1",$jpperpage,$jperpage,'lim',$lim);
	...
	$t.="	<div align=right style='margin-top:-30px'> ".str_replace("btn-mini","btn-xs",$lh)."</div>
	";
	$t.="<br>";
		...
		if ($showLinkOpr) {
			...
			if ($tbop=='') {
				...
				*/
				...
				href=# onclick=\"bukaAjax('content0','$hal&newrnd=$rnd&op=itb&id=$id".$addParamAdd."',0,'awalEdit($rnd);$addFuncEdit');
				return false\"><i class='icon-edit'></i></button>";
				$tbopr.="&nbsp;&nbsp;
				<button class='btn btn-danger btn-circle' href=# onclick=\"if (confirm('Yakin akan menghapus?')) { bukaAjax('content0','$hal&op=del&id=$id&newrnd=$rnd"."$addParamAdd');
				...
				*/
				$tbopr.="<button class='btn btn-info btn-circle' href=# onclick=\"bukaAjax('content0','inputcas.php?&nis=$id&newrnd=$rnd".$addParamAdd."','');return false\">
				<i class='icon-edit'></i></button>";
			}  elseif ($tbop=='rkegiatan') {//tv_$rnd
				/*
				$tbopr.="<button class='btn btn-danger btn-circle' href=# onclick=\"bukaAjaxD('tv_$rnd','inputcas.php?&nis=$id&newrnd=$rnd".$addParamAdd."','width:1000,height:500');return false\">
				<i class='icon-edit'></i></button>";
				*/
				$tbopr.="<button class='btn btn-info btn-circle' href=# onclick=\"bukaAjax('content0','inputrkegiatan.php?&nis=$id&newrnd=$rnd".$addParamAdd."','');return false\">
				<i class='icon-edit'></i></button>";
			}  elseif ($tbop=='cpres') {
				$tbopr.="&nbsp;&nbsp;<a href=# onclick=\"bukaAjax('content0','input.php?det=cpres&kode_kelas$kode_kelas&nis=$r[nis]&op=itb&id=$r[nis]".$addParamAdd."',0,'');
				return false\"><img src='img/icon/edit.png' title='Catatan Prestasi'></a>";
			}  elseif ($tbop=='spindah') {
		//	$tbopr.="&nbsp;&nbsp;<a href=# onclick=\"bukaAjax('content0','inputpindah.php?&op=ipindah&id=$id".$addParamAdd."',0,'');
			//	return false\"><img src='img/icon/move.png' title='Pindah Sekolah'></a>";
				$tbopr="&nbsp;&nbsp;<a href=# onclick=\"bukaAjax('content0','input.php?det=spindah&kode_kelas$kode_kelas&nis=$id&op=itb&id=$r[idp]".$addParamAdd."',0,'');
				return false\"><img src='img/icon/move.png' title='Pindah Sekolah'></a>";
				//$tbopr="-";
			}
		}
	...
	$tbtambah="";
		if ($tbop=='') {
			if ($showLinkTambah) {
				...
				$tbtambah="<br><button class='btn btn-success btn-sm '  href=# onclick=\"bukaAjax('content0','$hal&op=itb".$addParamAdd."',0,'$addFuncAdd');return false\"   >
