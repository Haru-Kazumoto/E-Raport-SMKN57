...
<?php
	...
	if (($det=="sekolah")||($det=="")) {
		include $toroot."inputsekolah.php";
	} else {
		include $toroot."input.php";
...
//include "input.php";
 ...
 while ($rn=mysql_fetch_array($hn)) {
	...
	*/
	...
	$tbopr.="&nbsp;&nbsp;<button class='btn btn-sm btn-circle btn-danger' href=# onclick=\"if (confirm('Yakin akan menghapus?')) { bukaAjax('content0','$hal&op=del&id=$idnews&newrnd=$rnd1');
...
if ($isAdmin){
	...
	href=# onclick=\"bukaAjax('content0','input.php?det=news&op=itb&newrnd=$rnd',0,'awalEdit($rnd)');return false;\" ><i class='icon-plus'></i> Tambah Info</button><br>";
}
 echo $t;
