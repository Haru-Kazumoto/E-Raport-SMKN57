...
$jdlTabel='';
	...
	while ($r=mysql_fetch_array($hq)) {
		...
		for($y=0;$y<$jField;$y++) {
			$nmField=$aField[$y];
			$t.="<td align=left class=tdoddform2 ><a href=# onclick=bukaAjax('content0','input.php?det=siswa&kode_kelas=$r[kode]');>".$r[strtolower($nmField)]."</a></td>";
		}
	...
	$t.="</table>";
	//$t.="<a href=# onclick=\"bukaAjax('content0','$hal?op=itb');return false\"   ><i class='icon-edit'></i>&nbsp;Tambah data $nmTabel</a>";
	$infoKelas=$t;
