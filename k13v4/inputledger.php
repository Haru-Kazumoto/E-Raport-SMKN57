...
if ($media=='print') {
	...
	*/
	...
	while ($r=mysql_fetch_array($h)) { 
	  ...
	  $tline.="<tr>"; 
	  $tline.="<td align=center >".($br+1)."</td>";
	  $line[]=$br+1;
	  
	  $xnis=$r[$aField[0]];
	  $xnama=$r[$aField[1]];
	  $i=0;$tline.="<td align=center > ".$xnis."</td>";
	  $i++;$tline.="<td > ".$xnama."</td>";
	  ...
	  $line[]=$r[$aField[1]];
		...
		foreach ($aKDMP as $kdmp) {					
			...
			if ($kdawal!='') {
				$syw=" kode_kompetensi like '$kdmp"."$kdawal"."$vsemester%' and nis='$r[nis]' and semester='$semester' ";
			...
			*/
			...
			//if ($kdmp=='C322') echo "<br>$jnil $jkom2 ";
			...
			$i++;$tline.="<td align=center $sty> $hasil</td>";
			$line[]=$hasil;
		...
		if ($jnilai=='Predikat') {
			...
			$i++;$tline.="<td align=center $sty >$predikat</td>";
			$line[]=$predikat;

		} else {
			if ($rata==0) $rata="&nbsp;";
			$i++;$tline.="<td align=center $sty> $rata</td>";
			$line[]=$rata;
		...
		if ($jki==4) {
			$sakit=$izin=$alpha="-";
			extractRecord("select sakit,izin,alpha from cas where semester='$semester' and nis='$xnis'");
			$line[]=$alpha;
			$line[]=$izin;
			$line[]=$sakit;
	...
	//urukan
	//echo $urutanledger;
	if ($urutanledger=='Peringkat') {
		$ad=($jnilai=="Nilai Murni"? SORT_DESC: SORT_ASC);
		$columns = array_column($arrData, $jKolLine-1);
	...
	foreach($arrData as $r) {
		...
		$t.="<tr>";
		$t.="<td align=center $sty >".($br+1)."</td>";
		
		foreach ($line as $dt) {
			$t.="<td align=center $sty>$dt</td>";
		}
		$t.="</tr>";
		$br++;
	}
...
*/
	$jnx='Nama,Peringkat';
	echo um412_isiCombo5("R:$jnx",'urutanledger');
	?></td> 
</tr><?php
/*
<tr class=troddform2 $sty >
