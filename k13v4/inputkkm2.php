...
extractRequest();
cekVar("media,asf,semester,op,kdmp");
if ($op=='simpan'){
	...
	for ($i=1;$i<=6;$i++) {
		$sfield.=($sfield==""?"":",");
		$sfield.="jp$i,jp$i"."a,kbp$i,desp$i,desp2$i";
		$sfield.=",jk$i,jk$i"."a,kbk$i,desk$i,desk2$i";
	}
	//echo $sfield;
	$ss="";
	...
	foreach ($afield as $fld) {
		eval("$"."isi=str_replace(\"'\",\"`\",$"."$fld);");
		$ss.=($ss==""?"":",")."$fld='$isi'";
	}
	
	$sq="update matapelajaran set $ss where kode='$kdmp'";
	eval("$"."ev=\"$sq\";");
	mysql_query($ev);
	//echo $ev."<br>";
...
if ($op=='showdata') {
	extractRecord("select * from matapelajaran where kode='$kdmp'");
	$r=$row;
	$nmFieldID="kode";
	$showLinkLihat=false;
	...
	$nfAction="inputkkm2.php?op=simpan&kdmp=$kdmp";
	$t="<div id=ts"."$idForm ></div>
	<form id='$idForm' action='$nfAction' method=Post $asf class=formInput target=_blank>
	";

	$t.="
	
	$ai
	<br><center><input type=submit value='Simpan Data' class='btn btn-primary btn-sm'></center></form>";
	echo$t;
	exit;
}
if ($media=='print') {
	...
	<div class=judul3 align=center>TAHUN AJARAN $thpl4 </div>
	<br>
	<br>
...
<div class=titlepage >Input KB/KKM</div> 
<table>

<tr class=troddform2 $sty >
	<td class=tdcaption >Jenis</td>
	<td><div id=tkelas_<?=$rnd?> >
	
	<?php
	
	
	echo um412_isiCombo5("$sJenisKdMP",'jenisMP','kode','nama','-Pilih-','',"gantiComboKKM('jenismp',$rnd)");
?>
	</tr>
<tr class=troddform2 $sty >
	...
	<td><div id=tmp_<?=$rnd?> >
	<?php
	$sq=$combogsmp." order by nama ";
	
	echo um412_isiCombo5($sq,'kode_matapelajaran','kode','nama','-Pilih-','',"gantiComboKKM('isi',$rnd)");
	?>
	</tr><?php 
