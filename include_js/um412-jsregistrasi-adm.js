...
function cekNama2(nama,rnd ) {
	...
	if (nama.length>=2) {	
		bukaAjax(tempat,'index.php?page=pilihnama2&newrnd='+rnd+'&nama='+nama+'&ch1='+ch1+'&jenis='+jenis+'&tempat='+tempat);
	} else {
...
function stf22(jenisx,idr,opr){
	...
	if (((bank=="pilih")||(bank=="")) && (opr=="confirm")) { //||(hp=="")||(cp=="")||(email=="")
		...
		url="index.php?page=stf&useJS=2&"+url;
		if (opr=='del') {
...
function aksiScanQR(kode){
	$("#noreg").val(kode);
	cekSelfRereg('selfrereg');	
}
*/
//cetaknametag
function cetakKartu(jeniscetak,rnd){
	...
	idreg=encodeURI(document.getElementById('idreg').value);
	//titlewks=encodeURI(document.getElementById('titlewks').value);
	titlewks=(document.getElementById('titlewks').value).replaceAll("&","aanndd");
	...
	neg=encodeURI(document.getElementById('neg').value);
	try { pospn=encodeURI(document.getElementById('pospn_'+rnd).value); } catch(e) {pospn=1; }
	pes='';
	if (rrd=='') pes+='\nPeserta belum reregistrasi';
	if (pes!='') {
	   alert(pes);
	  ...
	  // w=openWin(um_path+nfrereg+'?idres='+idres+'&nama='+nama+'&idreg='+idreg+'&title='+title+'&sebagai='+encodeURI(sebagai)+'&titlewks='+titlewks+'&jenis='+jenis+'&useBarcode='+useBarcode+'&neg='+neg, 'kartu', 800, 500, true); 	 
		nfrereg=toroot+"adm/index.php?rep=rereg-card&useJS=2&contentOnly=1"+"&no="+jeniscetak+"&pospn="+pospn; 
		w=openWin(nfrereg+'?idres='+idres+'&nama='+nama+'&namasert='+namasert+'&idreg='+idreg+'&title='+title+'&sebagai='+encodeURI(sebagai)+'&titlewks='+titlewks+'&jenis='+jenis+'&useBarcode='+useBarcode+'&neg='+neg, 'kartu', 800, 500, true); 	 
	   w.focus(); 
	}
}
...
function cetakSertifikat(no){
	...
	*/
	...
	idreg=encodeURI(document.getElementById('idreg').value);
	titlewks=encodeURI(document.getElementById('titlewks').value);
	title=(document.getElementById('title').value).replaceAll("&","aanndd");
	jenis=encodeURI(document.getElementById('jenis').value);
	...
	if (pes!='') {
	   ...
	   w=openWin(toroot+'adm/?rep=rereg-cert&contentOnly=1&useJS=2&idres='+idres+'&nama='+namasert+'&idreg='+idreg+'&title='+title+'&titlewks='+titlewks+'&jenis='+jenis+'&certas='+certas+'&certno='+certno+'&no='+no+'&skp='+skp, 'kartu', 1000, 500, true); 	 
	   w.focus();
	}
}
...
function isiAlamatEmail(tempat,jpeserta,sy){
	...
	url='index.php?rep=mail-isialamatemail&useJS=2&contentOnly=1&jpeserta='+jpeserta+'&sy='+encodeURI(sy);
	bukaAjax(tempat,url,2);
}
		...
		for (x=0;x<jlhreg;x++) {
			...
			if (nam!='-') {
				if  ((aidreg[x]==cari) || (nam.indexOf(cari.toLowerCase())>=0)){
					...
					if (aresreg[x].trim()=="") troe+=" tr_nores";
...
function bukaDetailReg(idreg){
	bukaAjax('tbiodata2',"index.php?rep=rereg_ac&idreg="+idreg+"&contentOnly=1&useJS=2");	
	document.getElementById('trereg').innerHTML='';
	needClearRereg=true;
	return false;
};
...
function cekHasilFooting(jh,ds){
		...
		if (ds==undefined) ds='';
		url='index.php?page=cekHasilFooting&desk='+ds+'&jhasil='+jh+'&jhasil='+jh;
...
function monitorHasilFooting(mh,desk){	
	...
	if (mhf==1) setTimeout("monitorHasilFooting("+mh+",'"+ds+"')",10000);
}
...
function cekFooting(m,desk) {
	...
	if (no=='test') {
		...
		url='index.php?page=cekFooting&no='+no+'&desk='+desk;
...
function cekDetailKandidatFooting(no,desk) {
	url='index.php?page=cekDetailKandidatFooting&no='+no+'&desk='+desk;
...
function cekAbsensi() {
	...
	if (idreg=='test') {
		...
		url='index.php?page=cekAbsensi&idreg='+idreg+'&nmacara='+nmacara+'&nmruang='+nmruang;
		//alert(url);
...
function pilihTempatAcara(rnd){
	...
	bukaAjaxD('tgtempat','index.php?page=pilihTempatAcara&tempatacara='+tempat,'width:600,height:500','');
	h=screen.height-70;
...
function cekSelfRereg(jform) {
	...
	if (idreg=='test') {
		...
		//nama=anamareg[idreg*1];
		//$('#tnama1').html(nama);
		url='index.php?page=cekSelfRereg&jform='+jform+'&idreg='+idreg+"&imd="+imd;
...
function absenLive(tempat){
	
	 $.ajax({
		 url:'index.php?contentOnly=1&useJS=2&rep=absen-daf&op=a'
	 }).done(function(data){
		try {
			d=document.getElementById(tempat);
			if (d==null) {
				console.log('absensi live berhenti');
			} else {
				$('#'+tempat).html(data);
				console.log(data);
				setTimeout('absenLive("'+tempat+'")',5000);	
			}
		} catch(e){
			console.log('absensi live berhenti');
		}			
