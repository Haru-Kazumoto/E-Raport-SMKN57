...
elseif ($op=='ed') {
	...
	foreach ($aField as $jFld) {
		...
		elseif ($jFld=="kepsek_tt")  {
			$nf=uploadFile($nmvar='kepsek_tt',$folderTarget="foto",$tipe="all",$maxfs=0,$nfonly=0,$nmfTarget="kepsek_tt_".$thpl.".ext");		
...
elseif ($op=='del') {
 ...
 elseif ($op=='itb') {
	...
	//$t.="<br><a href=# onclick=\"bukaAjax('content','$hal".$addParamAdd."');return false;\">Lihat Daftar $nmCaptionTabel</a>";
	$t.="</form>";
	echo $t;
	exit;
}
...
if ($op=='its') {
	...
	if ($userType=='admin') {
		$t.="<a href=# onclick=\"bukaAjax('content','$hal&op=itb&id=$id".$addParamAdd."',0,'$addFuncEdit');
