...
@media print {
	...
	.page {
		...
		".
	($ub=="Firefox"?
		"padding:1.7cm 1.3cm 1cm 0.1cm;"
	:
	"padding:2cm 1cm 1cm 0.1cm;"
	)
	."
	}
}
...
function cekPB($jbr=0,$force=false){
	...
	||($sy1 && $sy3&&($tbhead==''))||$force
		...
		if ($tbhead!='') $t.="</table>";
		//$t.="$jdes1-$jdes2 > $jdescoba1-$jdescoba2 pindah";
		$t.="</div><div ".$clspage." >".$tbhead;
		$jdes1=$jdescoba1-$jdes1+($tbhead==''?0:$charperbr*3);
		$jdes2=$jdescoba2-$jdes2+($tbhead==''?0:$charperbr*3);
	} else {
		//$t.="$jdes1-$jdes2 > $jdescoba1-$jdescoba2 masih lanjut";
		$jdes1=$jdescoba1;
		$jdes2=$jdescoba2;
	}
	//else $t.="$jdes1 $jdes2 >";
...
$tbl_foot[$x]=' 
		...
		<tr>
		  ...
		  <td   >&nbsp;</td>
		  <td  width="240"  valign="top"  align="center" >'.$webmasterCity.', '.$tglcetak.'</td>
		</tr>
		<tr>
		  <td  align="center">'.$tt[$x][0].'</td>
		  <td  align="center">&nbsp;</td>
		  <td  align="center" valign="top">'.$tt[$x][3].'</td>
		</tr>
		<tr>
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
		...
		<tr>
		  <td  align="center">'.$tt[$x][1].'</td>
		  <td  align="center">&nbsp;</td>
		  <td  align="center" valign="top">'.$tt[$x][4].'</td>
		</tr>
		<tr>
		  <td align="center" >'.$tt[$x][2].'</td>
		  <td>&nbsp;</td>
		  <td align="center" valign="top">'.$tt[$x][5].'</td>
		</tr>
	  </table>
	  ';
}
...
if ($printpage4) {
	...
	$w=hitungSkala(array(5,20,8,8,8,40,8,8,8,40),$wt,0);
...
<td align="center" width="'.$w[5].'">Deskripsi</td>
	...
	foreach ($aKDMP as $kdmp) {
		...
		if ($jknp1==0) {
			echo um412_falr("Mata pelajaran $kdmp- $mpg (pengetahuan) jumlah kd belum diisi....");
			exit;
		}
		if ($jknp2==0) {
			echo um412_falr("Mata pelajaran $kdmp- $mpg (keterampilan) jumlah kd belum diisi....");
			exit;
		}
		//Pengetahuan--------------------------------------------------------------------------------------------------------------------------------------
		$aKdAwal=explode("P","K");
		...
		//if ($jmid==1) {
			...
			$jknpIsi=$jknp1;
			//if ($jknpIsi==0) die($mp."->$kdmp->".$sqd);
			$np=$jnp/max($jknpIsi,1); //jika nilai dihitung dari seluruh mp
		//}
		...
		if ($jinput=='perkd') {
			...
			//$des1.="test test testtest test testtest test testtest test testtest test testtest test testtest test testtest test testtest test testtest test testtest test testtest test testtest test testtest test testtest test testtest test testtest test testtest test test";
			$des1=potong($des1,$maxdes,true,$strdst);//maks 500 huruf
	...
	//	$des2=$desx;
		...
		if ($jinput=='perkd') {
			...
			$des2=potong($des2,$maxdes,true,$strdst); 
	...
	if ($jmid==0) {
		...
		<table width="100%" border="1" class="tbcetakbergaris"  cellspacing="0" cellpadding="2">
		  <tr>
			<td style="padding:20px 10px"  >'.$catatan.'&nbsp;</td>
		  </tr>
		</table>';
		...
		<table width="100%" border="1" class="tbcetakbergaris"  cellspacing="0" cellpadding="2">
		  <tr>
			<td style="padding:20px 10px" >'.$tanggapanortu.'&nbsp;</td>
		  </tr>
		</table>
		';
		$tbl_capaian.=cekPB(6);
