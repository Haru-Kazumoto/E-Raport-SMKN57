...
if ($nocetak==0) {
	...
	if ($jcetak=='Raport'){
		if ($jmid!=1) {//mid semester
			...
			$printpage6=(isset($_REQUEST['pcetak_4'])?true:false);//cas
...
extractMapMP("kode_kelas='$kode_kelas' and semester='$semester'",$xAgama);

$aW=array(100,300,80,100);
$tbl_head1='<div style="font-size:11px">
...
//================================hal 1
	...
	<p style="font-size:20px;font-weight:bold;margin-top:150px">
	'.$judulRaport.'</p>
...
//================================hal 2
	...
	//$pdf->AddPage();
	$html= '<p align="center" style="font-size:20px;font-weight:bold;">
...
//	$shtml.=$tbl;
*/
	$shtml.="<div class=page>$html $tbl</div>";	
...
if ($printpage3) {
	...
	$html = '<p style="font-size:14px;font-weight:bold">PETUNJUK PENGGUNAAN</p><br>';
	$html.= '
...
$html='
	...
	<table cellspacing="2" cellpadding="1" border=0 width=100% >
	  ...
	  <tr>
		...
		$html.='</td>
		<td>'.$webmasterCity.", ".SQLtotgl($tanggal_terima).'</td>
	  </tr>
	  <tr>
...
if ($printpage4) {
	...
	if ($jmid!=1) {
		$catatan_mapel=cariField("select catatan_mapel from cas where nis='$nis' and semester='$semester'");
		
		if ($jPenilaian=='perkd') {
			//if (trim($catatan_mapel)=='') $catatan_mapel=cekCatatanMapel($nis,$semester,0);
		}
	...
	//SIKAP
	if ($jmid!=1) {
		$xsikap=carifield("select snilai from nilai_sikap where nis='$nis' and semester='$semester'");
		$tbl_capaian.='
	...
	$tbl_capaian.='
	<h3>'.$aAbjad[$sub].'. PENGETAHUAN DAN KETERAMPILAN</h3>
	...
	$tbhead='
		...
		for ($n=1;$n<=2;$n++) {
		  ...
		  <td align="center" style="width:'.$w[2].'px">Angka</td>
		  <td align="center" style="width:'.$w[2].'px">Predikat</td>
		  <td align="center" style="width:'.$w[3].'px">Deskripsi</td>
		  ';
}
		 $tbhead.=' </tr>
		  ';
	...
	foreach ($aKDMP as $kdmp) {
		...
		//kkm
		$kb1=carifield("select kbp"."$semester from matapelajaran where kode='$kdmp' ")*1;
		$kb2=carifield("select kbk"."$semester from matapelajaran where kode='$kdmp' ")*1;
		...
		$kdAwal="P";
		$synilai="from nilai_kompetensi_siswa inner join kompetensi on  nilai_kompetensi_siswa.kode_kompetensi=kompetensi.kode where nilai_kompetensi_siswa.kode_kompetensi like '$kdmp"."$kdAwal%' and nis='$nis' and kompetensi.semester=$semester";
		
		$jnp=cariField("select sum($dinilaiPg) $synilai  ");
		$jknp=max(1,cariField("select count(kode) from kompetensi where kode like '$kdmp"."$kdAwal%' and semester='$semester' ")*1);
		$np=$jnp/$jknp;
		if ($jmid==1) {
	...
	//	$des2=$desx;
		...
		if ($jbaris>=$batasbrakhir) {
			...
			* /
			//$shtml.=$tbl_capaian;
			$shtml.="<div class=page-landscape>$tbl_capaianl</div>";	
		...
		if ($jenis_mp!=$jenisMP) {
			...
			if ($kjmp!="") $kjmp=" : ".$kjmp;
				if ($jenis_mp=='C1 (Dasar Bidang Keahlian)') {
					$jbaris++;
					$tbl_capaian.='<tr><td colspan="11">Kelompok C : '.$kompetensi_keahlian.'</td>'.($nojmp==1?$isiCam:'').'</tr>';				
					}
				$jbaris++;
				$tbl_capaian.='<tr><td colspan="11">'.$jenis_mp.'</td>'.($nojmp==1?$isiCam:'').'</tr>';
				/*
				if ($jbaris>=$batasbrakhir) {
					$tbl_capaian.="</table>";
					$tbl_capaian=str_replace("#camap#",$komentar_am,$tbl_capaian);	
	...
	//$tbl_capaian=str_replace("#camap#",$komentar_am,$tbl_capaian);	
	//$tbl_capaian=str_replace("#jmp#",$jbaris,$tbl_capaian);	
	extractRecord("select * from cas where nis='$nis' and semester='$semester'");	
	...
	if (($semester%2==0) && ($jmid!=1)) {
		$html.='
		<br />
		<table width="100%" border=0 cellspacing="0" cellpadding="10">
		  <tr>
		  <td width="50%" valign="top">Wali Kelas,<br /><br /><br /><br />
			<u>'.$walikelas_nama.'</u><br />NIP.'.$walikelas_nip.'
			<br /><br /><br />Orang Tua/Wali<br /><br /><br /><br />'.$wali_nama.'
		  </td>
			<td valign="top"  style="border:1px solid #000;padding:5px;"><b>Keputusan:</b><br /><br />
				Berdasarkan hasil yang dicapai pada semester '.($semester-1).' dan '.$semester.', Siswa dinyatakan <b>'.$keputusan.'</b>. 
				<br /><br /><br />'.$webmasterCity.', '.$tglcetak.'<br /><br />
				Kepala Sekolah<br /><br /><br /><br /><u>'.$kepsek_nama.'</u><br />NIP.'.$kepsek_nip.'
			</td>
		  </tr>
		</table>
		<br><br><br />';
	 ...
	 //if ($jmid!=1) 
	 $tbl_capaian.=$html;
	 if ($semester%2==1) $tbl_capaian.=$tbl_foot[0]."";
	/*
	//echo $tbl_capaian;
	$pdf->setPrintHeader(false);
	$pdf->setPrintFooter(false);
...
if ($printpage5b) {
	...
	foreach ($aKDMP as $kdmp) {
		...
		if ($jenis_mp!=$jenisMP) {
			...
			$kjmp=$ketJMP[$nojmp-1];
			if ($kjmp!="") $kjmp=" : ".$kjmp;
			$tbl_deskripsi.='<tr><td colspan="7" >'.$jenis_mp.$kjmp.'</td></tr>';
		} //jenismp g sama
		...
		if ($mp!=$mpLama) {
			$mpLama=$mp;
			$nomp++;
			$tbl_deskripsi.='<tr><td colspan="7" >'.$mp.'</td></tr>';		 
		} //jenismp g sama
		...
		$desx="";$i=0;
		//kompetensi dasar===================================================================
		$skd="select * from kompetensi where semester='$semester' and kode_matapelajaran='$kdmp' order by ki desc ";
		$hkd=mysql_query($skd);
		$i=0;
		...
		while ($rkd=mysql_fetch_array($hkd)) {		
			...
			if ($kdAwalLama==$kdAwal) {
				...
				//echo "<br>select kb".$kdAwalLama.$semester." from matapelajaran where kode='$kdmp'";
				$kkm=carifield("select kb".$kdAwalLama.$semester." from matapelajaran where kode='$kdmp'");
				$jrsp=carifield("select count(kode) from kompetensi where kode_matapelajaran='$kdmp' and ki='$ki'");
