...
if ($printpage4) {
	...
	foreach ($aKDMP as $kdmp) {
		...
		//echo "<br>select sum($dinilaiPg) $synilai ";
		...
		if ($jenis_mp!=$jenisMP) {
			...
			if ($kjmp!="") $kjmp=" : ".$kjmp;
				if ($jenis_mp=='C1 (Dasar Bidang Keahlian)') {
					$jbaris++;
					$tbl_capaian.='<tr><td colspan="10">Kelompok C : '.$kompetensi_keahlian.'</td>'.($nojmp==1?$isiCam:'').'</tr>';				
					}
				$jbaris++;
				$tbl_capaian.='<tr><td colspan="10">'.$jenis_mp.'</td>'.($nojmp==1?$isiCam:'').'</tr>';
				/*
				if ($jbaris>=$batasbrakhir) {
					$tbl_capaian.="</table>";
					$tbl_capaian=str_replace("#camap#",$komentar_am,$tbl_capaian);	
