...
function setNilaiKompetensi() {
	global $xnis,$kode_matapelajaran,$ki,$n,$kk,$nilai,$semester;
	$dg="00000000".$n;
	//$kk=$kode_matapelajaran.substr($ki,0,1).substr($dg,-4);
	$kk=$kode_matapelajaran.substr($ki,0,1).$semester.substr($dg,-4);
	$sqcek="select count(nilai)  from nilai_kompetensi_siswa where nis='$xnis' and kode_kompetensi='$kk'  ";
	//echo "<br>$sqcek";
	/*
	if ($nilai=='') {
		$sqin="delete from   nilai_kompetensi_siswa where nis='$xnis' and kode_kompetensi='$kk' ";
	}
	else {
	*/
	$c=carifield($sqcek);
...
if ($op=='import') {
	cekVar("op2");
	if ($op2=='') {
		$addurl0="newrnd=$rnd&semester=$semester&kode_kompetensi=$kode_kompetensi&kode_kelas=$kode_kelas&ki=".urlencode($ki)."&semester=$semester&kode_matapelajaran=$kode_matapelajaran";
		...
		<div id=ts$idForm></dif>
		<form id=$idForm $asf action='$nfAct' method=post  enctype='multipart/form-data' target=_blank>  
		<table align=center ><tr>
		<td>
		<a href='inputnilai.php?op2=unduh&$addurl0&op=import&useJS=2' target=_blank style='color:#fff'><input  class='btn btn-warning btn-sm' type=button value='Unduh format'></a>
		</td><td>
		...
		*/
		...
		echo "Unggah data selesai, silahkan klik <a href=# onclick=\"bukaAjax('tnilai_$rnd','$backurl')\" >refresh</a>";
		//echo $t;
		exit;
	}
}  
if ($op=='showdata') {
	...
	if ($jn==0) {
		...
		echo "<a href=# onclick=bukaAjax('content','input.php?det=mata+pelajaran&op=itb&id=$kode_matapelajaran');>Klik di sini</a> untuk memasukkan
		";
		exit;
	}
	...
	$idForm="fnilai1";
	$asf="onsubmit=\"ajaxSubmitAllForm('$idForm','ts"."$idForm','');return false;\""; 
	$nfAction="inputnilai.php?op=simpan&semester=$semester&kode_kompetensi=$kode_kompetensi";
