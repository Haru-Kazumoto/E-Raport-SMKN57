...
class login {
	...
	function userLogout() {
		...
		$userid=$this->userid=$this->userid=$_SESSION["usrid"]="Guest";
		$userName=$this->userName=$_SESSION["usrnm"]="Guest";
		$userType=$this->userType=$_SESSION["usrtype"]="Guest";
		$vidusr=$_SESSION["vidusr"]=0;
	...
	function cekLogin($optcek=1) {
		global $levelOwner,$userid,$userName,$userType,$defKdGuru,$vidgu,$vidusr,$isLogin,$isTest;
		$newLogin=false;
		$uid=$ups="";
		...
		if (isset($_REQUEST["usrid"])) {
			...
			//simpan sesi
				...
				$_SESSION["usrlo"]=$this->levelOwner;
				//$_SESSION["usrstartedlogin"]=date('YmdHis');
		...
		elseif (isset($_SESSION["usrid"])) {	
			...
			$levelOwner=$this->levelOwner=$_SESSION["usrlo"];
