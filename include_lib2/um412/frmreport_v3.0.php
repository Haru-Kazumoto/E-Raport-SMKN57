...
if (isset($aFilterTb)) {
	...
	foreach ($aFilterTb as $sf) {
		...
		$sfldtb=$ofld[0];
		if (strstr($sfldtb,"-")!='') {
			$fldtb="concat(" . str_replace("-",",",$sfldtb).")";
		} else 
			$fldtb=$sfldtb;
		...
		if ((strstr($fld,"button")!='')||(strstr($fld,"checkbox")!='')) {
			...
			*/
			...
			$strInp=str_replace("#inp#","<input type=text name=x$fld id=x$fld"."_$rnd #onk# size=10>",$strInp);
			$addTbOpr2.=$strInp;	
		}
	}
	//$halDT=$urlR0;
	...
	$addTbOpr2=str_replace("#onk#","onkeyup=\"#url#\" ",$addTbOpr2);
	$addTbOpr2=str_replace("#onc#","onchange=\"#url#\" ",$addTbOpr2);
	$addTbOpr2=str_replace("#url#","reloadDT($rnd,'$urlR0');",$addTbOpr2);
	//$('#tbumum$rnd').DataTable().columns.adjust().draw();
}
...
if (($isTest)&&($op3!='json'))	$sHFilterTabel.="000>".$sqlall;
//cek media
//echo "<br>".$sqlall;
...
*/
	...
	$addfbe.="
		...
		console.log('dataTable$rnd.loaded');
		...
		function customDTHeight(rnd) {
			...
			h=window.hMax;
			h1=$('.dataTables_scrollHead').outerHeight();
			hhead2=$('.dataTables_scrollHead').height();
			h2=$('.dataTables_length').height();
			h3=$('.dataTables_breadcrumb').height();
			hhead=h1+h2;
			//alert('h2  '+h2+' h3 '+h3)
			if (h>400) {
...
if (strlen($sHFilterTabel)>10) {
		$sHFilterTabel=" 
	<div class='dataTables_breadcrumb breadcrumb2' id=bc_$rnd >$sHFilterTabel</div>
...
<span id=thalDT_$rnd  >$halDT</span>
<div id=thaltkn_$rnd ></div>
