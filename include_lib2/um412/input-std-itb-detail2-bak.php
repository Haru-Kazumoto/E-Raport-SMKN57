...
if (!isset($classtb)) {
	...
	$opcari="cari2";
	if (!isset($sSzFldD)) {
		$sSzFldD	="4,6,20,10,20,7,7,10,5,10,7";//ukuran input
		$detCari="lab"; 
		$fldCari="kode,deskripsi,panel_id,panel,satuan,nilaistd,brutto,disc,netto";
		$adf="hitungTransLab($rnd)";
		$sFuncFldD=",,,,,,,$adf,$adf,$adf,,,,,,,,,,";
		$sqd="select pdd.id,pdd.kode,lab.deskripsi,lab.nilaistd,lab.panel_id,lab.panel,lab.satuan,lab.brutto,lab.disc,lab.netto, pdd.hasillab,pdd.ket
		from tbpendaftaran pd inner join tbpendaftarand pdd on pd.id=pdd.idpendaftaran
		inner join tblab lab on pdd.kode=lab.kode and pdd.grp='lab' and pd.id='$idpd'"; 
		$sJenisFldD=",,,,,,i,,i,,,,,,,,,,,,";
	}
	$sqd=$sqTabelD;
}
...
*/
	...
	$headTb="<table border='1' cellpadding='1' cellspacing='1' style='width:100%' class='tb-$classtb' align=center >";
	...
	foreach ($aJudulKol as $judulKol) {
		...
		$sSample.="<td class='col-$k'style='$st' ><div id='$idtb-#br#-$k' >";
		if ($k==1) 	
		...
		elseif ($k==$jlhKol) 	{
			...
			//if ($jenisFld=='i')
				$sSample.="<input tipe=text class='$clsFldD' name='$classtb"."_"."$nmFldD"."[]' id='$classtb-$nmFldD-$rnd-#br#' idx='#br#' valuex='#v$nmFldD#'  size='$szFldD' $ro $xFunc >";
			//else
			//	$sSample.="#v$nmFldD#";
